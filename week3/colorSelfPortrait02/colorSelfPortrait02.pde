...
void setup()
{
  ...
  ColorList randomList = t.getColors(30);
  ...
  for (Iterator i = randomList.iterator(); i.hasNext();) {
    ...
    rect(xPos, 400, rectSize/2, rectSize);
    ellipse(xPos, 430, rectSize, rectSize);
    rect(xPos, 460, rectSize/2, rectSize);
    rect(xPos, 470, rectSize, rectSize);

    xPos += rectSize + spacing;
    yPos += rectSize + spacing;
  }
  endRecord();
}
